...
#[test]
fn v1_snapshot_from_v0_15_2_is_loadable() {
    // Generated by litesvm-persistence v0.15.2 at commit 2c4534a from
    // LiteSVM::default(), with compute-unit limit 123456, the removed V1
    // modular-exponentiation fields set to 654321/321, Poseidon A set to 789,
    // sigverify enabled, and log limit 4242.
    let bytes = include_bytes!("fixtures/v1_snapshot_v0_15_2.bin");
    let restored = from_bytes(bytes).unwrap();
    let compute_budget = restored.get_compute_budget().unwrap();

    assert_eq!(compute_budget.compute_unit_limit, 123_456);
    assert_eq!(compute_budget.poseidon_cost_coefficient_a, 789);
    assert!(restored.get_sigverify());
    assert_eq!(restored.get_log_bytes_limit(), Some(4_242));
}
